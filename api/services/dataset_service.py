...
class DatasetService:
    ...
    def get_datasets(page, per_page, provider="vendor", tenant_id=None, user=None, search=None, tag_ids=None):
        if user:
            permission_filter = db.or_(Dataset.created_by == user.id, Dataset.permission == "all_team_members")
        else:
    ...
    def create_segment(cls, args: dict, document: Document, dataset: Dataset):
        ...
        if dataset.indexing_technique == "high_quality":
            ...
            tokens = model_type_instance.get_num_tokens(
                model=embedding_model.model, credentials=embedding_model.credentials, texts=[content]
            )
