    ...
    def __init__(self, **data: Any) -> None:
        ...
        super().__init__(
            **{
                ...
                "credentials_schema": provider_yaml["credentials_for_provider"]
    ...
    def validate_parameters(self, tool_id: int, tool_name: str, tool_parameters: dict[str, Any]) -> None:
        ...
        for parameter in tool_parameters_need_to_validate:
            ...
            if parameter_schema.default is not None:
                ...
                if (
                    parameter_schema.type == ToolParameter.ToolParameterType.STRING
                    or parameter_schema.type == ToolParameter.ToolParameterType.SELECT
                ):
        ...
        for credential_name in credentials:
            ...
            if (
                ...
                if not isinstance(credentials[credential_name], str):
                    raise ToolProviderCredentialValidationError(
                        f"credential {credential_schema.label.en_US} should be string"
                    )
                ...
                if not isinstance(options, list):
                    raise ToolProviderCredentialValidationError(
                        f"credential {credential_schema.label.en_US} options should be list"
                    )

                if credentials[credential_name] not in [x.value for x in options]:
                    raise ToolProviderCredentialValidationError(
                        f"credential {credential_schema.label.en_US} should be one of {options}"
                    )
                ...
                if isinstance(credentials[credential_name], bool):
                    ...
                    if credentials[credential_name].lower() == "true":
                        ...
                        raise ToolProviderCredentialValidationError(
                            f"credential {credential_schema.label.en_US} should be boolean"
                        )
