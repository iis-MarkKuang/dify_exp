...
class CodeNode(BaseNode):
    ...
    def _run(self, variable_pool: VariablePool) -> NodeRunResult:
        ...
        for variable_selector in node_data.variables:
            ...
            value = variable_pool.get_variable_value(variable_selector=variable_selector.value_selector)

            variables[variable] = value
        # Run code
        try:
            result = CodeExecutor.execute_workflow_code_template(
                language=code_language, code=code, inputs=variables, dependencies=node_data.dependencies
            )
    ...
    def _check_string(self, value: str, variable: str) -> str:
        ...
        if len(value) > MAX_STRING_LENGTH:
            raise ValueError(
                f"The length of output variable `{variable}` must be less than {MAX_STRING_LENGTH} characters"
            )
    ...
    def _check_number(self, value: Union[int, float], variable: str) -> Union[int, float]:
        ...
        if value > MAX_NUMBER or value < MIN_NUMBER:
            raise ValueError(
                f"Output variable `{variable}` is out of range, it must be between {MIN_NUMBER} and {MAX_NUMBER}."
            )
        ...
        if isinstance(value, float):
            ...
            if len(str(value).split(".")[1]) > MAX_PRECISION:
                raise ValueError(
                    f"Output variable `{variable}` has too high precision, it must be less than {MAX_PRECISION} digits."
                )
                ...
                if isinstance(output_value, dict):
                    ...
                    if first_element is not None:
                        if isinstance(first_element, int | float) and all(
                            isinstance(value, int | float) for value in output_value
                        ):
