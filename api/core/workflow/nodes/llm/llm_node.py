    ...
    def _run(self, variable_pool: VariablePool) -> NodeRunResult:
        ...
        try:
            ...
            return NodeRunResult(
                status=WorkflowNodeExecutionStatus.FAILED, error=str(e), inputs=node_inputs, process_data=process_data
            )

        outputs = {"text": result_text, "usage": jsonable_encoder(usage)}
            ...
            if isinstance(context_value, str):
                ...
                if self.callbacks and original_retriever_resource:
                    for callback in self.callbacks:
                        callback.on_event(
                            event=QueueRetrieverResourcesEvent(retriever_resources=original_retriever_resource)
                        )
