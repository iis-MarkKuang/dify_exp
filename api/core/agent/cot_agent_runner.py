...
class CotAgentRunner(BaseAgentRunner, ABC):
            ...
            if not scratchpad.action:
                ...
                if scratchpad.action.action_name.lower() == "final answer":
                    ...
                    tool_invoke_response, tool_invoke_meta = self._handle_invoke_action(
                        action=scratchpad.action, tool_instances=tool_instances, message_file_ids=message_file_ids
                    )
    ...
    def _handle_invoke_action(
        self, action: AgentScratchpadUnit.Action, tool_instances: dict[str, Tool], message_file_ids: list[str]
        ...
        for message_file, save_as in message_files:
            ...
            self.queue_manager.publish(
                QueueMessageFileEvent(message_file_id=message_file.id), PublishFrom.APPLICATION_MANAGER
            )
    ...
    def _organize_historic_prompt_messages(
        ...
        for message in self.history_prompt_messages:
            if isinstance(message, AssistantPromptMessage):
                ...
                if scratchpad:
                    result.append(AssistantPromptMessage(content=self._format_assistant_message(scratchpad)))
