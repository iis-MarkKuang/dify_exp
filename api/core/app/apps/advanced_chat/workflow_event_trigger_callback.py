...
class WorkflowEventTriggerCallback(BaseWorkflowCallback):
    ...
    def on_workflow_run_started(self) -> None:
        ...
        """
        self._queue_manager.publish(QueueWorkflowStartedEvent(), PublishFrom.APPLICATION_MANAGER)

    def on_workflow_run_succeeded(self) -> None:
        """
        Workflow run succeeded
        """
        self._queue_manager.publish(QueueWorkflowSucceededEvent(), PublishFrom.APPLICATION_MANAGER)

    def on_workflow_run_failed(self, error: str) -> None:
        """
        Workflow run failed
        """
        self._queue_manager.publish(QueueWorkflowFailedEvent(error=error), PublishFrom.APPLICATION_MANAGER)
    ...
    def on_node_text_chunk(self, node_id: str, text: str, metadata: Optional[dict] = None) -> None:
        ...
        """
        self._queue_manager.publish(
            QueueTextChunkEvent(text=text, metadata={"node_id": node_id, **metadata}), PublishFrom.APPLICATION_MANAGER
        )
    ...
    def on_workflow_iteration_next(
        self, node_id: str, node_type: NodeType, index: int, node_run_index: int, output: Optional[Any]
    ) -> None:
        """
        ...
        self._queue_manager._publish(
            QueueIterationNextEvent(
                node_id=node_id, node_type=node_type, index=index, node_run_index=node_run_index, output=output
            ),
            PublishFrom.APPLICATION_MANAGER,
        )
    ...
    def on_workflow_iteration_completed(
        ...
        self._queue_manager._publish(
            QueueIterationCompletedEvent(
                node_id=node_id, node_type=node_type, node_run_index=node_run_index, outputs=outputs
            ),
            PublishFrom.APPLICATION_MANAGER,
        )
