...
class WorkflowLoggingCallback(BaseWorkflowCallback):
    ...
    def on_workflow_node_execute_succeeded(
        ...
        self.print_text(
            f"Metadata: {jsonable_encoder(execution_metadata) if execution_metadata else ''}", color="green"
        )
    ...
    def on_workflow_iteration_next(
        self, node_id: str, node_type: NodeType, index: int, node_run_index: int, output: Optional[dict]
    ) -> None:
        """
