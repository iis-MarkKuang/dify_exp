...
class VolcengineMaaSLargeLanguageModel(LargeLanguageModel):
    ...
    def _generate(
        ...
        req_params = ModelConfigs.get(credentials["base_model_name"], {}).get("req_params", {}).copy()
        ...
        resp = MaaSClient.wrap_exception(lambda: client.chat(req_params, prompt_messages, stream))
        if not stream:
    ...
    def _handle_stream_chat_response(
        self, model: str, credentials: dict, prompt_messages: list[PromptMessage], resp: Generator
    ) -> Generator:
    ...
    def _handle_chat_response(
        self, model: str, credentials: dict, prompt_messages: list[PromptMessage], resp: dict
    ) -> LLMResult:
    ...
    def get_customizable_model_schema(self, model: str, credentials: dict) -> AIModelEntity | None:
        ...
        """
        max_tokens = ModelConfigs.get(credentials["base_model_name"], {}).get("req_params", {}).get("max_new_tokens")
        if credentials.get("max_tokens"):
