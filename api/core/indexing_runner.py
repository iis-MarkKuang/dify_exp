...
class IndexingRunner:
    ...
    def _get_splitter(
        self, processing_rule: DatasetProcessRule, embedding_model_instance: Optional[ModelInstance]
    ) -> TextSplitter:
        """
            ...
            if dataset.indexing_technique == "high_quality" or embedding_model_type_instance:
                tokens += sum(
                    embedding_model_type_instance.get_num_tokens(
                        embedding_model_instance.model, embedding_model_instance.credentials, [document.page_content]
                    )
    ...
    def _update_document_index_status(
        self, document_id: str, after_indexing_status: str, extra_update_params: Optional[dict] = None
    ) -> None:
        """
