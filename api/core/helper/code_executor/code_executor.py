...
class CodeExecutor:
    ...
    def list_dependencies(cls, language: str) -> list[CodeDependency]:
        ...
        with cls.dependencies_cache_lock:
            cls.dependencies_cache[language] = {"data": dependencies, "expiration": time.time() + 60}
    ...
    def _get_dependencies(cls, language: Literal["python3"]) -> list[CodeDependency]:
        ...
        try:
            ...
            if response.status_code != 200:
                raise Exception(
                    f"Failed to list dependencies, got status code {response.status_code}, please check if the sandbox service is running"
                )
            response = response.json()
